document.addEventListener('DOMContentLoaded', function init(){
    ...
    fetch('https://randomuser.me/api/?inc=gender,name,nat,email,picture,age&results=20')
    .then(function(response) {
      return response.json();
     })
    .then(user => {
        friends = user.results;
        // show(friends);
      console.log([friends[0].name.first,friends[0].name.last].join("-")); // iliakan
    });

    function displayFriends(user){
     let friendBox = user.map(item => {
       return `<div  class="friendContainer">
       <div class="info">
          <div  class="person">
                ...
                <div class="about_person">
                     <p class="person_name">${item.name.first} ${item.name.last}</p>
                     <p class="person_gender">${item.gender}</p>
                     <p class="person_age"> ${item.age}</p>
                     <p class="person_nationality">${item.nat}</p>
                     <p class="person_mail">${item.email}</p>
                  </div>
            </div>
          </div>
       </div>`}).join(" ");
     container.innerHTML = friendBox;
    };
